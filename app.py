...
if q2_response in ["잘 알고 있다 (이름과 역할을 이해한다)", "들어본 적은 있다 (이름은 알지만 역할은 잘 모른다)"]:
    ...
    if q2_1_response != "전혀 조작해본 적 없고 자동 모드만 사용한다":
        st.subheader("2-1-1. 세 가지 요소 중에서 가장 조절하기 어렵다고 느낀 것은 무엇인가요?")
        q2_1_1_response = st.radio("q2_1_1", ("조리개", "셔터 속도", "ISO", "잘 모르겠다"), label_visibility="collapsed")
...
# TODO: 아래 이미지 URL을 실제 이미지 파일 경로로 변경하세요 (예: "my_image_1.png")
col1, col2, col3 = st.columns(3)
with col1:
    st.image("https://placehold.co/300x300/AAAAAA/FFFFFF?text=초점/선명도", caption="사진 A")
with col2:
    st.image("https://placehold.co/300x300/CCCCCC/FFFFFF?text=색감", caption="사진 B")
with col3:
    st.image("https://placehold.co/300x300/EEEEEE/FFFFFF?text=구도", caption="사진 C")

q4_response = st.radio("q4", ("초점 / 선명도", "색감", "구도"), label_visibility="collapsed")
...
q5_response = st.radio(
    "q5",
    ("구도", "색감", "초점", "ISO, 조리개, 셔터스피드 값 조정", "기타"),
    label_visibility="collapsed"
)
st.subheader("5-1. 예상과 실제 결과물이 다른 경우 어떻게 하시나요?")
q5_1_response = st.radio(
    "q5_1",
    ("삭제", "후보정", "촬영방법에 대한 공부", "주변에 물어보기", "기타"),
    label_visibility="collapsed"
)
st.divider()

st.subheader("6. 사진을 찍은 후 어떤 부분에서 아쉬움을 느끼셨나요? (2개 선택)")
q6_response = st.multiselect(
    "q6",
    ["화질이 마음에 들지 않음", "색감이 별로임", "구도가 생각한대로 되지 않음", "초점이 맞지 않음", "특정 부분이 밝게 날아감", "기타"],
    max_selections=2,
    label_visibility="collapsed"
)
st.divider()
...
q7_response = st.radio(
    "q7",
    ("원하는 사진촬영에 대한 방법", "일기예보 확인", "촬영스팟 검색", "카메라 사진 폰으로 옮기기", "활용하지 않음", "기타"),
    label_visibility="collapsed"
)
st.divider()
...
if submit_button:
    try:
        ...
        new_data = {
            ...
            "6. 아쉬운 점(최대 2개)": ", ".join(q6_response), # 리스트를 문자열로 변환
